...
class LifeFormTableViewController: UITableViewController {
    ...
    func fetchMatchingItems() {
        ...
        if !searchTerm.isEmpty {
            let searchRequest = SearchAPIRequest(searchTerm: searchTerm)
            searchTask?.cancel()
            searchTask = Task {
                do {
                    let searchResults = try await LifeFormController.shared.sendRequest(searchRequest)
    ...
    @IBSegueAction func showItemDetails(_ coder: NSCoder, sender: Any?) -> LifeFormItemViewController? {
        guard let cell = sender as? UITableViewCell, let indexPath = tableView.indexPath(for: cell) else {
            return nil
        }
        
        let item = items[indexPath.row]
        return LifeFormItemViewController(coder: coder, item: item)
    }
